{% extends "base.html" %}
{% block content %}
{% set sections = sections if sections is defined else ['monitoring','problem','discovery','maintenance','email','access','agent','webhooks','reports','failure_alerts','capture','interface'] %}
<h1 class="page-title">{{ settings_title if settings_title is defined else 'Settings' }}</h1>
...
{% if 'access' in sections and is_admin %}
<div class="card">
  <h2>Access control <span class="muted">(admin only)</span></h2>
  <p class="card-sub">Restrict who can sign in and enforce two-factor authentication.</p>
  <div class="toggle-line">
    <div class="t-text"><div class="t-title">Require 2FA for all users</div>
      <div class="t-desc">Everyone must set up an authenticator app before they can use the dashboard.</div></div>
    <label class="switch"><input type="checkbox" data-key="require_2fa"><span class="track"></span></label>
  </div>
  <div style="margin-top:14px;">
    <label class="f-label">Allowed email addresses</label>
    <textarea data-key="allowed_emails" rows="4" placeholder="*@ascom.com&#10;noc@partner.com&#10;*@*.ascom.co.uk"
      style="width:100%;padding:8px 10px;border-radius:7px;border:1px solid var(--baseline);background:var(--surface);color:var(--ink);font:inherit;resize:vertical;"></textarea>
    <div class="f-help">One pattern per line (or comma-separated). Wildcards allowed:
      <code>*@ascom.com</code>, <code>*@*.ascom.co.uk</code>. A user whose account email
      doesn't match any pattern is denied access. <b>Leave blank to allow all emails.</b>
      Accounts with a blank email are always allowed (so the local <code>ascom</code> admin never locks out).</div>
  </div>
</div>
{% endif %}
  ...
  <div class="f-row">
    <div style="flex:1;">
      <label style="display:inline-flex;align-items:center;gap:8px;">
        <input type="checkbox" id="agent-insecure">
        <span class="f-label" style="margin:0;">Allow self-signed certificate</span>
      </label>
      <div class="f-help">Tick only if the controller uses HTTPS with a self-signed
        certificate and you get a <code>CERTIFICATE_VERIFY_FAILED</code> error. This
        skips TLS certificate verification for the controller connection — use only
        on a trusted network. If the controller is plain HTTP, leave this off.</div>
    </div>
  </div>
  <div style="display:flex;gap:8px;flex-wrap:wrap;">
...
(function () {
  ...
  async function load() {
    ...
    if (document.getElementById("agent-enabled")) try {
      ...
      document.getElementById("agent-insecure").checked = !!ag.config.insecure;
  ...
  document.getElementById("save-btn").addEventListener("click", async () => {
    ...
    try {
      ...
      // save agent-mode config too (only if that section is on this page)
      if (document.getElementById("agent-enabled")) {
        await api("/api/agent", { method: "POST", body: JSON.stringify({
          ...
          insecure: document.getElementById("agent-insecure").checked,
        }) });
      }
  ...
  const agentTestBtn = document.getElementById("agent-test");
  if (agentTestBtn) agentTestBtn.addEventListener("click", async ev => {
  ...
  if (testEmailBtn) testEmailBtn.addEventListener("click", async ev => {
    const btn = ev.target;
  ...
  if (testWebhookBtn) testWebhookBtn.addEventListener("click", async ev => {
    const btn = ev.target;
